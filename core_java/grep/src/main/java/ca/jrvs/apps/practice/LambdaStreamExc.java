...
public interface LambdaStreamExc {
	...
	/**
	 * Create a String stream from array of strings.
	 *
	 * note: arbitrary number of value will be stored in an array.
	 *
	 * @param strings on which the createStream operation is performed.
	 ...
	 *
	 * @param strings on which the upper-case operations will be performed.
	 ...
	 * return another stream which no element contains a
	 *
	 *
	 * @param stringStream on which filter operation is performed.
	 * @param pattern      to check against the string of operations.
	 * @return stream of strings.
	 */
	 ...
	 * 
	 * @param arr on which an createIntStream function will be performed.
	 * @return an Int of streams.
	 */
	 ...
	 * 
	 * @param intStream which will be converted to List of Integers.
	...
	/**
	 * Return a lambda function that print a message with a prefix and suffix This
	 * lambda can be useful to format logs
	 *
	 * You will learn: - functional interface http://bit.ly/2pTXRwM &
	 * http://bit.ly/33onFig - lambda syntax
	 *
	...
	/**
	 * Print all odd number from a intStream. Please use `createIntStream` and
	 * `getLambdaPrinter` methods
	 *
	 * e.g. lse.printOdd(lse.createIntStream(0, 5), lse.getLambdaPrinter("odd
	 * number:", "!"));
	 *
	...
	/**
	 * Square each number from the input. Please write two solutions and compare
	 * difference - using flatMap
	 *
	 * @param ints
	 * @return
	 */

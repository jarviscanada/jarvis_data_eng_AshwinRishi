...
public class TwitterDao implements CrdDao<Tweet, String> {
	...
	private URI formatPostUri(Tweet tweet) throws URISyntaxException {
		...
		return new URI(BASE_URI + POST_URI + QUERY + "status" + EQUAL + percentEscaper.escape(tweet.getText())
				+ AMPERSAND + "long" + EQUAL + percentEscaper.escape(String.valueOf(coordinates.get(0))) + AMPERSAND
				+ "lat" + EQUAL + percentEscaper.escape(String.valueOf(coordinates.get(1))));
	}
